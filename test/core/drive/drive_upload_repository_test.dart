...
void main() {
  group('DriveUploadRepository', () {
    ...
    test('getPendingJobs includes failed jobs whose retry time has passed',
        () async {
      final db = _db();
      addTearDown(db.close);
      final repo = DriveUploadRepository(db);
    ...
    test('explicit Upload All retries failed and exhausted jobs', () async {
      final db = _db();
      addTearDown(db.close);
      final repo = DriveUploadRepository(db);
      ...
      expect(j2.status, DriveUploadJobStatus.pending);
      expect(j1.retryCount, 0);
      expect(j1.failureReason, isNull);
      expect(j2.failureReason, isNull);
      expect(await repo.getPendingJobs(), hasLength(2));
    });

    test('pending count excludes completed upload history', () async {
      final db = _db();
      addTearDown(db.close);
      final repo = DriveUploadRepository(db);
      for (final id in ['pending', 'done']) {
        await repo.insertJob(
          id: id,
          assignmentId: 'a1',
          filePath: '/$id.jpg',
      ...
      await repo.markCompleted('done', driveFileId: 'remote');
      expect(await repo.watchPendingCount().first, 1);
    });

    test('jobExistsForFilePath returns true when non-completed job exists',
        () async {
      final db = _db();
      addTearDown(db.close);
      final repo = DriveUploadRepository(db);
    ...
    test(
        'shapefileJobExistsForAssignment returns true while a job is in flight',
        () async {
      final db = _db();
      addTearDown(db.close);
      final repo = DriveUploadRepository(db);
    ...
    test(
        'shapefileJobExistsForAssignment returns false once the job is completed',
        () async {
    ...
    test(
        'shapefileJobExistsForAssignment returns false when the job is failed (re-enqueue should not be blocked)',
        () async {
    ...
    test('shapefileJobExistsForAssignment returns false once the job is dead',
        () async {
      final db = _db();
      addTearDown(db.close);
      final repo = DriveUploadRepository(db);
